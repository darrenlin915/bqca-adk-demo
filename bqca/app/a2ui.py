...
def _render_vega_png(spec: dict) -> bytes | None:
    ...
    try:
        ...
        warning_msg = str(e)
        logger.warning("vega-lite render failed: %s", warning_msg)
        return None
    ...
    for step in steps:
        for kind, payload in step.items():
            if kind in (
                ...
                # export button stays in A2UI because it's interactive.
                ...
                # than inline in the chat. See `bqca_thoughts` + agent.py's
...
def slides_to_a2ui(
    ...
    """Card with thumbnail images + URL text."""
    cid = _IdGen()
    components: list[dict] = []
    children: list[str] = []
...
def csv_to_a2ui(
    ...
    """Card with CSV metadata + URL text."""
    cid = _IdGen()
    components: list[dict] = []
    children: list[str] = []
