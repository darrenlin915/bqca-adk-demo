...
# Populated by the FastAPI middleware in fast_api_app.py from the inbound
# `Authorization: Bearer <token>` header. GE puts the end user's OAuth access
# token there when invoking an A2A agent that declares `agentAuthorization`.
ge_auth_token: ContextVar[Optional[str]] = ContextVar("ge_auth_token", default=None)
...
def _inject_ge_token(callback_context: CallbackContext) -> None:
    """Copy the per-request GE bearer token into ADK session state.

    `tools._negotiate_creds` already checks `state["temp:<AUTH_ID>"]` first
    thing (Stage 1), so writing it here lets the existing OAuth fallback path
    stay unchanged for the local-dev / ADK-on-Agent-Runtime cases.
    """
    token = ge_auth_token.get()
    if token:
        callback_context.state[f"temp:{auths.TOKEN_CACHE_KEY}"] = token
...
6. 全程使用繁體中文回覆。""",
    ...
    before_agent_callback=_inject_ge_token,
